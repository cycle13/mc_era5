...
TOPDIR = Path(getenv('ACCACIA')) / 'reanalysis'
LEVELS = ['500', '600', '700', '800', '850', '900', '925', '950', '975', '1000']
# VARNAMES = ['msl', 'u/v/vo']
LEVTYPES = {'sst': 'single', 'ci': 'single', 'msl': 'single', 'lsm': 'single',
            'u': 'pressure', 'v': 'pressure', 'vo': 'pressure'}
...
L.add(LOGFILE)
NC_SAVE_KW = dict(encoding={'time': {'units': 'hours since 1900-01-01 00:00:0.0'}},
                  unlimited_dims=('time',))
...
def download(req):
    ...
    for varname in VARNAMES:
        ...
        if req['levtype'] == 'pl':
            req['levelist'] = LEVELS
        fname = (f"{req['dataset']}.{req['type']}.{req['levtype']}"
                 f".{dt0:%Y%m%d}-{dt1:%Y%m%d}.nc")
...
def slice_files(fnames, req):
    ...
    for varname in ds.data_vars:
        for m in set(ds.time.dt.month.values):
            ...
            fname_out = (f"{req['dataset']}.{req['type']}.{LEVTYPES[varname]}"
...
"""
    ...
    ap.add_argument('-q', '--quiet', action='store_true',
                    help='Suppress log messages (write to log file only)')
    ap.add_argument('-y', '--years', required=True, type=str,
                    help='Years list (sep: ,) or span (sep: -)')
    ap.add_argument('-m', '--months', required=True, type=str,
                    help='Months list (sep: ,) or span (sep: -)')
    ap.add_argument('-n', '--names', required=True, type=str,
                    help='Comma-separated list of variables')
    ap.add_argument('-t', '--timestep', type=int,
                    required=True, choices=[1, 3, 6],
                    help='Time step (h)')

    ap.add_argument('--slice', action='store_true', default=False,
    ...
    for year in years:
        for month in months:
            for varname in varnames:
                ...
                fname = (f"{PRODUCT_NAME}.an.{LEV_ABBR[levtype]}"
                         f".{year}.{month:02d}.{varname}.nc")
                target = str(outdir / fname)
                L.info(f'target = {target}')
                C.retrieve(NAME.format(product_name=PRODUCT_NAME,
                                       levtype=levtype),
                           req,
                           target)
